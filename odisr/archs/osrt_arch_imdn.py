...
from odisr.archs.rfpoub3 import RepRFN, SCPA_rep, SCPA_rep_offset

from basicsr.utils.registry import ARCH_REGISTRY
from basicsr.archs.arch_util import to_2tuple, trunc_normal_, flow_warp, DCNv2Pack
from odisr.archs.architecture import IMDN
...
class OSRT(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_first = nn.Conv2d(num_in_ch, 64, 3, 1, 1)
    
        # self.offset_conv2 = nn.Sequential(nn.Conv2d(1, num_in_ch, 1, 1, 0, bias=True),
        #                                          nn.LeakyReLU(negative_slope=0.2, inplace=True),
        #                                          nn.Conv2d(num_in_ch, num_in_ch, 1, 1, 0, bias=True),
        #                                          nn.LeakyReLU(negative_slope=0.2, inplace=True))

        # self.conv_first2 = DCNv2Pack(3, 3, 1, padding=0)
        ...
        # # self.conv = BSConvU
        # if self.training:
        #     self.scpa1 = SCPA_rep(60,2,1,1,act_type='lrelu', deploy=False)
        #     self.scpa2 = SCPA_rep(60,2,1,1,act_type='lrelu', deploy=False)
        #     self.scpa3 = SCPA_rep(60,2,1,1,act_type='lrelu', deploy=False)
        #     self.scpa4 = SCPA_rep(60,2,1,1,act_type='lrelu', deploy=False)
        ...
        # else:
        #     self.scpa1 = SCPA_rep(60,2,1,1,act_type='lrelu', deploy=True)
        #     self.scpa2 = SCPA_rep(60,2,1,1,act_type='lrelu', deploy=True)
        #     self.scpa3 = SCPA_rep(60,2,1,1,act_type='lrelu', deploy=True)
        #     self.scpa4 = SCPA_rep(60,2,1,1,act_type='lrelu', deploy=True)
        ...
        # self.c3 = nn.Conv2d(60 * 8, 60, 1)
        # self.c4 = self.conv(60, 60, kernel_size=3, **kwargs)
        self.imdn = IMDN(in_nc=3, nf=64, num_modules=6, out_nc=3, upscale=2)
        ...
        if self.upsampler == 'pixelshuffledirect':
            ...
            self.upsample = UpsampleOneStep(upscale, 64, num_out_ch,
    ...
    def forward(self, x, condition):
        ...
        if self.upsampler == 'pixelshuffledirect':
            # for lightweight SR
            # x = self.conv_first(x)
            # print(self.offset_conv2(condition).shape, '11111')
            # print(condition.shape,'111111111')
            # x = self.conv_first2(x, self.offset_conv2(condition))
            x = self.conv_first(x)
            ...
            # x_fam1 = self.fam1(x)
            # out1 = self.scpa1(x)
            # out2 = self.scpa2(out1)
            # out3 = self.scpa3(out2)
            # out4 = self.scpa4(out3)
            # out5 = self.scpa5(out4)
            # out6 = self.scpa6(out5)
            # out7 = self.scpa7(out6)
            # out8 = self.scpa8(out7)

            # trunk = torch.cat([out1, out2, out3, out4, out5, out6, out7, out8], dim=1)
            # out_B = self.c1(trunk)
            # out_B = self.GELU(out_B)

            # out_lr = self.c2(out_B) + x

            # out9 = self.scpa9(out_lr)
