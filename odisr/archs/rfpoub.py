...
#         self.__delattdev1x1_sbx_branch')
...
class RepRFB(nn.Module):
    def __init__(self, feature_nums, act_type='lrelu', deploy=False):
        ...
        self.repblock1 = RepBlock(in_channels=feature_nums, out_channels=feature_nums, act_type=act_type, deploy=deploy)
        self.conv_change1 = nn.Conv2d(feature_nums*2, feature_nums, 3, 1, 1)
        ...
        self.repblock2 = RepBlock(in_channels=feature_nums, out_channels=feature_nums, act_type=act_type, deploy=deploy)
        self.conv_change2 = nn.Conv2d(feature_nums*2, feature_nums, 3, 1, 1)
        ...
        self.repblock3 = RepBlock(in_channels=feature_nums, out_channels=feature_nums, act_type=act_type, deploy=deploy)
        self.conv_change3 = nn.Conv2d(feature_nums*2, feature_nums, 3, 1, 1)
        ...
        self.conv_change4 = nn.Conv2d(feature_nums*2, feature_nums, 3, 1, 1)
    ...
    def forward(self, inputs):
        ...
        x_fam1 = self.fam1(inputs)
        outputs = self.conv_change1(torch.cat((outputs, x_fam1), dim=1))

        outputs = self.repblock2(outputs)
        x_fam2 = self.fam2(outputs)
        outputs = self.conv_change2(torch.cat((outputs, x_fam2), dim=1))

        outputs = self.repblock3(outputs)
        x_fam3 = self.fam3(outputs)
        outputs = self.conv_change3(torch.cat((outputs, x_fam3), dim=1))

        outputs = self.repblock4(outputs)
        x_fam4 = self.fam4(outputs)
        outputs = self.conv_change4(torch.cat((outputs, x_fam4), dim=1))
...
class RepRFN(nn.Module):
    def __init__(self,in_channels=3, out_channels=3, feature_nums=48, upscale_factor=4,
        ...
        # self.fam4 = FAM(in_channels=feature_nums, out_channels=feature_nums, kernel_size=3, stride=1, padding=1, bias=True, split=2, reduction=2)
        # self.reprfb4 = RepRFB(feature_nums=feature_nums, act_type='lrelu', deploy=deploy)
        # self.conv_change = nn.Conv2d(240, 60, 3,1,1)
    ...
    def forward(self, inputs):
        # outputs_feature = self.fea_conv(inputs)
        # outputs1 = self.scpa1(inputs)
        # outputs1_2 = self.fam1(inputs)
        outputs1 = self.reprfb1(inputs)
        # outputs1 = outputs1 + outputs1_2 + inputs
        
        # outputs = self.scpa1(outputs)
        # outputs = self.glob1(outputs)
        # outputs2_2 = self.fam2(outputs1)
        outputs2 = self.reprfb2(outputs1)
        # outputs2 = self.scpa2(outputs1)
        # outputs2 = outputs2 + outputs2_2 + outputs1
        
        # outputs = self.scpa2(outputs)
        # outputs = self.glob2(outputs)
        # outputs3_2 = self.fam3(outputs2)
        outputs3 = self.reprfb3(outputs2)
        # outputs3 = self.scpa3(outputs2)
        # outputs3 = outputs3 + outputs3_2 + outputs2
        
        # outputs = self.scpa3(outputs)
        # outputs = self.glob3(outputs)
        
        # output4_2 = self.fam4(outputs3)
        output4 = self.reprfb4(outputs3)
        # output4 = self.scpa4(outputs3)
        # output4= output4_2 + output4 + outputs3
        
        # outputs = self.reprfb4(outputs)
        # outputs = self.scpa4(outputs)
        # outputs = self.glob4(outputs)
        # outputs = torch.cat([outputs1, outputs2, outputs3,output4], dim=1)

        # outputs = self.conv_change(outputs)
        outputs = self.lr_conv(output4)
