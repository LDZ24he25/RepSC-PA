...
from odisr.archs.rfpoub import RepRFN
...
class Block(nn.Module):
    def __init__(self, 
        ...
        self.b1 = ops.ResidualBlock(60, 60)
        self.b2 = ops.ResidualBlock(60, 60)
        self.b3 = ops.ResidualBlock(60, 60)
        self.c1 = ops.BasicBlock(60*2, 60, 1, 1, 0)
        self.c2 = ops.BasicBlock(60*3, 60, 1, 1, 0)
        self.c3 = ops.BasicBlock(60*4, 60, 1, 1, 0)
...
class Net(nn.Module):
    def __init__(self, dim):
        ...
        # self.upsample = ops.UpsampleBlock(60, scale=scale, 
...
class SMSR(nn.Module):
    def __init__(self, conv=common.default_conv):
        ...
        self.collect = nn.Sequential(
            nn.Conv2d(60*5, 60, 1, 1, 0),
            nn.ReLU(True),
            nn.Conv2d(60, 60, 3, 1, 1)
        )

        # define tail module
        # modules_tail = [
        #     nn.Conv2d(n_feats, 3*self.scale*self.scale, 3, 1, 1),
...
class OSRT(nn.Module):
    ...
    def __init__(self,
        ...
        #         layer = Rescpa(
        #             in_channels=60, out_channels=60, act_type='lrelu', deploy=False
        #             )
        #         self.layers.append(layer)
        ...
        #     for i_layer in range(12):
        #         layer = Rescpa(
        #             in_channels=60, out_channels=60, act_type='lrelu', deploy=True
        #             )
        #         self.layers.append(layer)
        # if self.training:
        #     self.reprfn = RepRFN(in_channels=60, out_channels=60, feature_nums=60, upscale_factor=4,
        #             deploy=False)
        # else:
        #     self.reprfn = RepRFN(in_channels=60, out_channels=60, feature_nums=60, upscale_factor=4,
        #             deploy=True)
        self.cran = Net(dim=60)
        # self.conv_first8 = nn.Conv2d(60, 8, 3,1,1)
        # self.conv_last8 = nn. Conv2d(8, 60, 3,1,1)
    ...
    def forward(self, x, condition):
        ...
        if self.upsampler == 'pixelshuffledirect':
            # for lightweight SR
            x = self.conv_first(x)
        #     # x= self.conv_first8(x)

        #     # offset = self.offset_conv(condition).permute(0,2,3,1)
        #     # _x = flow_warp(x, offset, interp_mode='bilinear', padding_mode='border')
        #     # offset = self.offset_conv(condition)
        #     # for layer in self.layers:
        #     #     x = layer(x)
        #     x = self.reprfn(x)
        #     # x = self.conv_last8(x)
