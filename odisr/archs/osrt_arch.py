...
from odisr.archs.rfpoub8 import RepRFN, SCPA_rep, SCPA_rep2, RepBlock
...
def window_partition(x, window_size):
    ...
    """
    b, h, w, c = x.shape
    x = x.view(b, h // window_size, window_size, w // (window_size*4), window_size*4, c)
    windows = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, window_size, window_size*4, c)
    return windows
...
def window_reverse(windows, window_size, h, w):
    ...
    Returns:
        x: (b, h, w, c)
    """
    b = int(windows.shape[0] / (h * w / window_size / window_size / 4))
    x = windows.view(b, h // window_size, w // (window_size * 4) , window_size, window_size * 4, -1)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH
        # print(self.relative_position_bias_table.shape, '333')   #[456,6]
    ...
    def forward(self, xq, xkv=None, mask=None):
        ...
        # print(attn.shape,'aaaaaaaaaaa')#[640, 128, 128]

        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)  # Wh*Ww,Wh*Ww,nH
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
        # print(relative_position_bias.shape,'11111')
        # print(attn.shape,'2222')
        ...
        if mask is not None:
            ...
            # print(attn.shape, mask.shape,'4444')
            #[4096, 6,128,128]  [16384,128,128]
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = WindowAttention(
            dim,
            window_size=(self.window_size, self.window_size*4),
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            qk_scale=qk_scale,
            attn_drop=attn_drop,
    ...
    def calculate_mask(self, x_size):
        ...
        # print(img_mask.shape,'000')
        h_slices = (slice(0, -self.window_size), slice(-self.window_size,
                                                       -self.shift_size), slice(-self.shift_size, None))
        w_slices = (slice(0, -self.window_size*4), slice(-self.window_size*4,
                                                       -self.shift_size), slice(-self.shift_size, None))
        cnt = 0
        for h in h_slices:
            for w in w_slices:
        ...
        # print(mask_windows.shape, '090909')
        mask_windows = mask_windows.view(-1, self.window_size * self.window_size*4)
    ...
    def forward(self, x, x_size, condition):
        ...
        if self.use_vit_condition:
            x = x.permute(0, 3, 1, 2)
            if self.window_condition:
                condition_wind = torch.stack(torch.meshgrid(torch.linspace(-1,1,4),torch.linspace(-1,1,16)))\
                    .type_as(x).unsqueeze(0).repeat(b, 1, h//self.window_size, w//(self.window_size*4))
                if self.shift_size > 0:
            ...
            x_windows_warped = x_windows_warped.view(-1, self.window_size * self.window_size*4, c)  # nw*b, window_size*window_size, c
        else:
        ...
        x_windows = x_windows.view(-1, self.window_size * self.window_size*4, c)  # nw*b, window_size*window_size, c
        ...
        # merge windows
        attn_windows = attn_windows.view(-1, self.window_size, self.window_size*4, c)
...
class Repscpa(nn.Module):
    def __init__(self,
        ...
        self.rstb1 = SwinTransformerBlock(
                dim=embed_dim,
        ...
        self.rstb2 = SwinTransformerBlock(
                dim=embed_dim,
                input_resolution=img_size,
                num_heads=6,
                window_size=window_size,
        ...
        self.rstb3 = SwinTransformerBlock(
                dim=embed_dim,
                input_resolution=img_size,
                num_heads=6,
                window_size=window_size,
                shift_size=0 if (3 % 2 == 0) else window_size // 2,
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
                drop=0.,
        ...
        self.rstb4 = SwinTransformerBlock(
                dim=embed_dim,
                input_resolution=img_size,
                num_heads=6,
                window_size=window_size,
                shift_size=0 if (4 % 2 == 0) else window_size // 2,
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
                drop=0.,
        ...
        # else:
        self.scpa1 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=True)
        self.scpa2 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=True)
        self.scpa3 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=True)
        self.scpa4 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=True)

        self.bc1 = BSConvU(60, 30)
        self.bc2 = BSConvU(60, 30)
        self.bc3 = BSConvU(60, 30)
        self.bc4 = BSConvU(60, 30)



        self.c1 = nn.Conv2d(30*4, 30, 1)
        self.GELU = nn.GELU()
        self.c2 = self.conv(30, 30, kernel_size=3, **kwargs)
    ...
    def forward(self, x,condition):
        ...
        if self.use_dcn_conv:      
            ...
            x1 = self.pos_drop(x1)
            x1 = self.rstb1(x1, x_size, condition)
        ...
        if self.use_dcn_conv:      
            x2 = self.patch_embed(out1)
            if self.ape:
                x2 = x2 + self.absolute_pos_embed
            x2 = self.pos_drop(x2)
            x2 = self.rstb2(x2, x_size, condition)
        ...
        if self.use_dcn_conv:      
            x3 = self.patch_embed(out3)
            if self.ape:
                x3 = x3 + self.absolute_pos_embed
            x3 = self.pos_drop(x3)
            x3 = self.rstb3(x3, x_size, condition)
            x3 = self.norm(x3)  # b seq_len c
            x3 = self.patch_unembed(x3, x_size)
        # out3 = out3 + x3
        ...
        if self.use_dcn_conv:      
            x4 = self.patch_embed(out3)
            if self.ape:
                x4 = x4 + self.absolute_pos_embed
            x4 = self.pos_drop(x4)
            x4 = self.rstb4(x4, x_size, condition)
            x4 = self.norm(x4)  # b seq_len c
            x4 = self.patch_unembed(x4, x_size)
        # out4 = out4 + x4
        out4 = torch.cat((out4, x4),dim=1)
        out4 = self.bc4(out4)
...
class OSRT(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_first = nn.Conv2d(num_in_ch, 30, 3, 1, 1)
        ...
        if self.training:
            self.rep1 = RepBlock(30,30,act_type='lrelu', deploy=False)
            self.rep2 = RepBlock(30,30,act_type='lrelu', deploy=False)
            self.rep3 = RepBlock(30,30,act_type='lrelu', deploy=False)
            self.rep4 = RepBlock(30,30,act_type='lrelu', deploy=False)
        #     self.rep5 = RepBlock(30,30,act_type='lrelu', deploy=False)
        #     self.rep6 = RepBlock(30,30,act_type='lrelu', deploy=False)
        #     self.rep7 = RepBlock(30,30,act_type='lrelu', deploy=False)
        #     self.rep8 = RepBlock(30,30,act_type='lrelu', deploy=False)
        #     self.rep9 = RepBlock(30,30,act_type='lrelu', deploy=False)
        #     self.rep10 = RepBlock(30,30,act_type='lrelu', deploy=False)
        #     self.rep11 = RepBlock(30,30,act_type='lrelu', deploy=False)
        #     self.rep12 = RepBlock(30,30,act_type='lrelu', deploy=False)


        else:
            self.rep1 = RepBlock(30,30,act_type='lrelu', deploy=True)
            self.rep2 = RepBlock(30,30,act_type='lrelu', deploy=True)
            self.rep3 = RepBlock(30,30,act_type='lrelu', deploy=True)
            self.rep4 = RepBlock(30,30,act_type='lrelu', deploy=True)
        # self.rep5 = RepBlock(30,30,act_type='lrelu', deploy=True)
        # self.rep6 = RepBlock(30,30,act_type='lrelu', deploy=True)
        # self.rep7 = RepBlock(30,30,act_type='lrelu', deploy=True)
        # self.rep8 = RepBlock(30,30,act_type='lrelu', deploy=True)
        # self.rep9 = RepBlock(30,30,act_type='lrelu', deploy=True)
        # self.rep10 = RepBlock(30,30,act_type='lrelu', deploy=True)
        # self.rep11 = RepBlock(30,30,act_type='lrelu', deploy=True)
        # self.rep12 = RepBlock(30,30,act_type='lrelu', deploy=True)

        self.scpa1 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=False)
        self.scpa2 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=False)
        self.scpa3 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=False)
        self.scpa4 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=False)
        # self.scpa5 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=False)
        # self.scpa6 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=False)
        # self.scpa7 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=False)
        # self.scpa8 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=False)
        # self.scpa9 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=False)
        # self.scpa10 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=False)
        # self.scpa11 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=False)
        # self.scpa12 = SCPA_rep2(30,2,1,1,act_type='lrelu', deploy=False)
        ...
        # self.bc5 = BSConvU(60, 30)
        # self.bc6 = BSConvU(60, 30)
        # self.bc7 = BSConvU(60, 30)
        # self.bc8 = BSConvU(60, 30)

        # self.bc9 = BSConvU(60, 30)
        # self.bc10 = BSConvU(60, 30)
        # self.bc11 = BSConvU(60, 30)
        # self.bc12 = BSConvU(60, 30)

        self.c1 = nn.Conv2d(30*4, 30, 1)
        self.GELU = nn.GELU()
        self.c2 = self.conv(30, 30, kernel_size=3, **kwargs)

        # self.c3 = nn.Conv2d(30*4, 30, 1)
        # self.GELU = nn.GELU()
        # self.c4 = self.conv(30, 30, kernel_size=3, **kwargs)

        # self.c5 = nn.Conv2d(30*4, 30, 1)
        # self.GELU = nn.GELU()
        # self.c6 = self.conv(30, 30, kernel_size=3, **kwargs)
    ...
    def forward(self, x, condition):
        ...
        if self.upsampler == 'pixelshuffledirect':
            ...
            # out_lr = self.repscpa(x, condition)

            # out1 = self.scpa1(x,condition)
            out1 = self.rep1(x)
            if self.use_dcn_conv:      
                x1 = self.patch_embed(out1)
                # print(x1.shape, '111111')
                if self.ape:
                    x1 = x1 + self.absolute_pos_embed
                x1 = self.pos_drop(x1)
                x1 = self.rstb1(x1, x_size, condition)
                ...
                # print(x1.shape, '111111')
            # out1 = out1 + x1
            out1 = self.scpa1(out1,condition)
            out1 = torch.cat((out1, x1),dim=1)
            out1 = self.bc1(out1)
            ...
            out2 = self.rep2(out1)
            if self.use_dcn_conv:      
                x2 = self.patch_embed(out2)
                # print(x2.shape, '22222')
                if self.ape:
                    x2 = x2 + self.absolute_pos_embed
                x2 = self.pos_drop(x2)
                x2 = self.rstb2(x2, x_size, condition)
                ...
                # print(x2.shape, '22222')
            # out2 = out2 + x2
            out2 = self.scpa2(out2,condition)
            out2 = torch.cat((out2, x2),dim=1)
            out2 = self.bc2(out2)
            ...
            if self.use_dcn_conv:      
                ...
                x3 = self.patch_unembed(x3, x_size)
            # out3 = out3 + x3
            out3 = self.scpa3(out3,condition)
            out3 = torch.cat((out3, x3),dim=1)
            out3 = self.bc3(out3)
            ...
            if self.use_dcn_conv:      
                ...
                x4 = self.patch_unembed(x4, x_size)
            # out4 = out4 + x4
            out4 = self.scpa4(out4,condition)
            out4 = torch.cat((out4, x4),dim=1)
            out4 = self.bc4(out4)
            ...
            # out5 = self.rep5(out_lr)
            # if self.use_dcn_conv:      
            #     x5 = self.patch_embed(out5)
            #     if self.ape:
            #         x5 = x5 + self.absolute_pos_embed
            #     x5 = self.pos_drop(x5)
            #     x5 = self.rstb5(x5, x_size, condition)
            #     x5 = self.norm(x5)  # b seq_len c
            #     x5 = self.patch_unembed(x5, x_size)
            # # out1 = out1 + x1
            # out5 = self.scpa5(out5,condition)
            # out5 = torch.cat((out5, x5),dim=1)
            # out5 = self.bc5(out5)
            ...
            # out6 = self.rep6(out5)
            # if self.use_dcn_conv:      
            #     x6 = self.patch_embed(out6)
            #     if self.ape:
            #         x6 = x6 + self.absolute_pos_embed
            #     x6 = self.pos_drop(x6)
            #     x6 = self.rstb6(x6, x_size, condition)
            #     x6 = self.norm(x6)  # b seq_len c
            #     x6 = self.patch_unembed(x6, x_size)
            # # out2 = out2 + x2
            # out6 = self.scpa6(out6,condition)
            # out6 = torch.cat((out6, x6),dim=1)
            # out6 = self.bc6(out6)

            # # out3 = self.scpa3(out2,condition)
            # out7 = self.rep7(out6)
            # if self.use_dcn_conv:      
            #     x7 = self.patch_embed(out7)
            #     if self.ape:
            #         x7 = x7 + self.absolute_pos_embed
            #     x7 = self.pos_drop(x7)
            #     x7 = self.rstb7(x7, x_size, condition)
            #     x7 = self.norm(x7)  # b seq_len c
            #     x7 = self.patch_unembed(x7, x_size)
            # # out3 = out3 + x3
            # out7 = self.scpa7(out7,condition)
            # out7 = torch.cat((out7, x7),dim=1)
            # out7 = self.bc7(out7)
            ...
            # out8 = self.rep8(out7)
            # if self.use_dcn_conv:      
            #     x8 = self.patch_embed(out8)
            #     if self.ape:
            #         x8 = x8 + self.absolute_pos_embed
            #     x8 = self.pos_drop(x8)
            #     x8 = self.rstb8(x8, x_size, condition)
            #     x8 = self.norm(x8)  # b seq_len c
            #     x8 = self.patch_unembed(x8, x_size)
            # # out4 = out4 + x4
            # out8 = self.scpa8(out8,condition)
            # out8 = torch.cat((out8, x8),dim=1)
            # out8 = self.bc8(out8)
            ...
            # out9 = self.rep9(out_lr2)
            # if self.use_dcn_conv:      
            #     x9 = self.patch_embed(out9)
            #     if self.ape:
            #         x9 = x9 + self.absolute_pos_embed
            #     x9 = self.pos_drop(x9)
            #     x9 = self.rstb9(x9, x_size, condition)
            #     x9 = self.norm(x9)  # b seq_len c
            #     x9 = self.patch_unembed(x9, x_size)
            # # out4 = out4 + x4
            # out9 = self.scpa9(out9,condition)
            # out9 = torch.cat((out9, x9),dim=1)
            # out9 = self.bc9(out9)
            

            # out10 = self.rep10(out9)
            # if self.use_dcn_conv:      
            #     x10 = self.patch_embed(out10)
            #     if self.ape:
            #         x10 = x10 + self.absolute_pos_embed
            #     x10 = self.pos_drop(x10)
            #     x10 = self.rstb10(x10, x_size, condition)
            #     x10 = self.norm(x10)  # b seq_len c
            #     x10 = self.patch_unembed(x10, x_size)
            # # out4 = out4 + x4
            # out10 = self.scpa10(out10,condition)
            # out10 = torch.cat((out10, x10),dim=1)
            # out10 = self.bc10(out10)


            # out11 = self.rep11(out10)
            # if self.use_dcn_conv:      
            #     x11 = self.patch_embed(out11)
            #     if self.ape:
            #         x11 = x11 + self.absolute_pos_embed
            #     x11 = self.pos_drop(x11)
            #     x11 = self.rstb11(x11, x_size, condition)
            #     x11 = self.norm(x11)  # b seq_len c
            #     x11 = self.patch_unembed(x11, x_size)
            # # out4 = out4 + x4
            # out11 = self.scpa11(out11,condition)
            # out11 = torch.cat((out11, x11),dim=1)
            # out11 = self.bc11(out11)


            # out12 = self.rep12(out11)
            # if self.use_dcn_conv:      
            #     x12 = self.patch_embed(out12)
            #     if self.ape:
            #         x12 = x12 + self.absolute_pos_embed
            #     x12 = self.pos_drop(x12)
            #     x12 = self.rstb12(x12, x_size, condition)
            #     x12 = self.norm(x12)  # b seq_len c
            #     x12 = self.patch_unembed(x12, x_size)
            # # out4 = out4 + x4
            # out12 = self.scpa12(out12,condition)
            # out12 = torch.cat((out12, x12),dim=1)
            # out12 = self.bc12(out12)
