...
class SFF(nn.Module):
    def __init__(self, input_dim,hidden_dim,output_dim):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(2*hidden_dim, 2*hidden_dim, 3,1,1),
            nn.GELU(),
            nn.Conv2d(2*hidden_dim, output_dim,3,1,1),
            nn.GELU()
        )

        self.conv_fft1 = nn.Sequential(
            nn.Conv2d(input_dim, hidden_dim , 3,1,1),
            nn.GELU(),
    ...
    def forward(self,input):
        output_fft = torch.fft.fft2(input, dim=(-2,-1))
        output_fft_real = output_fft.real
        output_fft_imag = output_fft.imag

        output_fft_real = self.conv_fft1(output_fft_real)
        output_fft_imag = self.conv_fft2(output_fft_imag)
        # output_fft = torch.cat((output_fft_imag, output_fft_real), dim=1)
        output_fft = torch.stack((output_fft_real,output_fft_imag), dim=-1)

        output_fft = torch.fft.ifft2(torch.complex(output_fft[..., 0], output_fft[..., 1], dim=(-2,-1)))
        ...
        # output_fft = torch.fft.ifft2(output_fft)

        output = self.conv1(input)
        output = torch.cat((output, output_fft), dim=1)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.add_sff = SFF(dim, c_dim, dim)
    ...
    def forward(self, x, x_size, condition):
        ...
        x = x + self.drop_path(self.mlp(self.norm2(x))) + x1
