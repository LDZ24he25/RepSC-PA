...
class Partial_conv3(nn.Module):
    ...
    def __init__(self, dim, n_div, forward):
        ...
        self.dcn = DCNv2Pack(self.dim_conv3, self.dim_conv3, 3,1,1)
        self.offset_conv = nn.Sequential(nn.Conv2d(1, self.dim_conv3, 1, 1, 0, bias=True),
                                    nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                    nn.Conv2d(self.dim_conv3, self.dim_conv3, 1, 1, 0, bias=True),
                                    nn.LeakyReLU(negative_slope=0.2, inplace=True))
    ...
    def forward_split_cat(self, x: Tensor , condition) -> Tensor:
        # for training/inference
        x1, x2 = torch.split(x, [self.dim_conv3, self.dim_untouched], dim=1)
        condition = self.offset_conv(condition)

        x1 = self.dcn(x1, condition)
        x = torch.cat((x1, x2), 1)
...
class MLPBlock(nn.Module):
    ...
    def forward(self, x: Tensor,condition) -> Tensor:
        shortcut = x
        x = self.spatial_mixing(x,condition)
...
class RSTB(nn.Module):
    ...
    def __init__(self,
        ...
        self.mlpblock = MLPBlock(dim, 3, 0.5, drop_path=0.1, layer_scale_init_value=0, act_layer = 'RELU', norm_layer='BN', pconv_fw_type='split_cat')
