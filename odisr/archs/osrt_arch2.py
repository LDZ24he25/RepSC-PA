...
class OSRT(nn.Module):
    ...
    def __init__(self,
        ...
        # self.conv_f = nn.Conv2d(num_in_ch, 1, 3, 1, 1)
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.upsample = UpsampleOneStep(upscale, embed_dim + 128, num_out_ch,
    ...
    # def f_h(self, x):
    #     lpf = torch.zeros((x.shape[2], x.shape[3]))
    #     R = (x.shape[2] + x.shape[3]) // 8
    #     for i in range(x.shape[2]):
    #         for j in range(x.shape[3]):
    #             if ((i - (x.shape[2] - 1)/ 2) ** 2 + (j - (x.shape[3] -1)/2)**2) < (R **2):
    #                 lpf[j, i] =1
    #     hpf = 1-lpf
    #     self.hpf, self.lpf = hpf.cuda(), lpf.cuda()
    #     return self.hpf, self.lpf


        # x = fft.fftn(x, dim=(2, 3))
        # x = torch.roll(x, (x.shape[2]//2, x.shape[3]//2), dims=(2,3))
    ...
    def forward(self, x,condition):
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)    # shallow extraction
            ...
            x = torch.cat((x, x_manet), dim=1)
            x = self.upsample(x)
            # x_del = self.upsample(x_del_1)
            # x1 = x1 - x_del
        elif self.upsampler == 'nearest+conv':
            # for real-world SR
            x = self.conv_first(x)
    ...
    # def forward(self, x, condition):  # lq, condition

        lpf = torch.zeros((x.shape[2], x.shape[3])).cuda()
        R = (x.shape[2] + x.shape[3]) // 8
        for i in range(x.shape[2]):
            for j in range(x.shape[3]):
                if ((i - (x.shape[2] - 1)/ 2) ** 2 + (j - (x.shape[3] -1)/2)**2) < (R **2):
        ...
        hpf = hpf.cuda()
        x2 = fft.fftn(x, dim=(2, 3))
        x2 = torch.roll(x2, (x.shape[2]//2, x.shape[3]//2), dims=(2,3))
        x_l = x2 * lpf
        x_h = x2 * hpf
        x_h = torch.abs(fft.ifftn(x_h, dim=(2, 3)))
        x_l = torch.abs(fft.ifftn(x_l, dim=(2, 3)))
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            y = self.conv_first(x)    # shallow extraction
            # 傅里叶变换提取高频信息
            y = y + x_h
            if self.use_dcn_conv:
                _x = self.conv_after_body(self.forward_features(y, condition))
                x1 = self.dcn(_x , self.offset_conv(condition)) + y
            else:
                x1 = self.conv_after_body(self.forward_features(y, condition)) + y
            x1 = self.conv_before_upsample(x1)
            x1 = self.conv_last(self.upsample(x1))
        elif self.upsampler == 'pixelshuffledirect':
            # for lightweight SR
            x3 = x + x_h
            x_del_1 = self.conv_first(x_l)
            # y = F.normalize(x3, dim=(2, 3))
            y = self.conv_first(x3)
            # y = y + x_h
