...
from odisr.archs.rfpoub6 import RepRFN, SCPA_rep, SCPA_rep2, RepBlock

from basicsr.utils.registry import ARCH_REGISTRY
from basicsr.archs.arch_util import to_2tuple, trunc_normal_, flow_warp, DCNv2Pack
# from odisr.archs.mamba import Mamba
...
class OSRT(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_first = nn.Conv2d(num_in_ch*4, 60, 3, 1, 1)
        # self.conv_first = BSConvU(12, 60)
        ...
        # self.mamba1 = Mamba(
        #         d_model=60, # Model dimension d_model
        ...
        # if self.training:
        #     self.rep1 = RepBlock(60,60,act_type='lrelu', deploy=False)
        #     self.rep2 = RepBlock(60,60,act_type='lrelu', deploy=False)
        #     self.rep3 = RepBlock(60,60,act_type='lrelu', deploy=False)
        #     self.rep4 = RepBlock(60,60,act_type='lrelu', deploy=False)
        ...
        # else:
        self.rep1 = RepBlock(60,60,act_type='lrelu', deploy=True)
        self.rep2 = RepBlock(60,60,act_type='lrelu', deploy=True)
        self.rep3 = RepBlock(60,60,act_type='lrelu', deploy=True)
        self.rep4 = RepBlock(60,60,act_type='lrelu', deploy=True)
        ...
        # self.bc1 = BSConvU(30, 30)
        # self.bc2 = BSConvU(30, 30)
        # self.bc3 = BSConvU(30, 30)
        # self.bc4 = BSConvU(30, 30)

        # self.bc5 = BSConvU(60, 60)
        ...
        self.para1 = torch.nn.Parameter(torch.zeros(1))
        self.para2 = torch.nn.Parameter(torch.zeros(1))
        self.para3 = torch.nn.Parameter(torch.zeros(1))
        self.para4 = torch.nn.Parameter(torch.zeros(1))
        ...
        if self.upsampler == 'pixelshuffledirect':
            ...
            self.upsample = UpsampleOneStep(upscale, 60, num_out_ch,
    ...
    def forward(self, x, condition):
        ...
        if self.upsampler == 'pixelshuffledirect':
            ...
            if self.use_dcn_conv:      
                ...
                x1 = self.patch_unembed(x1, x_size)
            # out1 = out1 + x1
            out1 = self.scpa1(out1[:,30:60,:,:],condition)
            out1 = torch.cat((out1, x1),dim=1)
            out1 = self.para1 * x + out1
            # out1 = self.bc1(out1)
            ...
            out2 = self.rep2(out1)
            if self.use_dcn_conv:      
                x2 = self.patch_embed(out2[:,30:60,:,:])
                if self.ape:
                    x2 = x2 + self.absolute_pos_embed
                x2 = self.pos_drop(x2)
                x2 = self.rstb2(x2, x_size, condition)
                ...
                x2 = self.patch_unembed(x2, x_size)
            # out2 = out2 + x2
            out2 = self.scpa2(out2[:,0:30,:,:],condition)
            out2 = torch.cat((out2, x2),dim=1)
            out2 = self.para2 * x + out2
            # out2 = self.bc2(out2)

            # out3 = self.scpa3(out2,condition)
            out3 = self.rep3(out2)
            if self.use_dcn_conv:      
                x3 = self.patch_embed(out3[:,0:30,:,:])
                if self.ape:
                    x3 = x3 + self.absolute_pos_embed
                x3 = self.pos_drop(x3)
                x3 = self.rstb3(x3, x_size, condition)
                ...
                x3 = self.patch_unembed(x3, x_size)
            # out3 = out3 + x3
            out3 = self.scpa3(out3[:,30:60,:,:],condition)
            out3 = torch.cat((out3, x3),dim=1)
            out3 = self.para3 * x + out3
            # out3 = self.bc3(out3)
            ...
            out4 = self.rep4(out3)
            if self.use_dcn_conv:      
                x4 = self.patch_embed(out4[:,30:60,:,:])
                if self.ape:
                    x4 = x4 + self.absolute_pos_embed
                x4 = self.pos_drop(x4)
                x4 = self.rstb4(x4, x_size, condition)
                ...
                x4 = self.patch_unembed(x4, x_size)
            # out4 = out4 + x4
            out4 = self.scpa4(out4[:,0:30,:,:],condition)
            out4 = torch.cat((out4, x4),dim=1)
            out4 = self.para4 * x + out4
            # out4 = self.bc4(out4)
