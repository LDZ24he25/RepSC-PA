...
def window_partition(x, window_size):
    ...
    """
    b, h, w, c = x.shape
    x = x.view(b, h // window_size, window_size, w // (window_size*2), window_size*2, c)
    windows = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, window_size, window_size*2, c)
    return windows
...
def window_reverse(windows, window_size, h, w):
    ...
    x = windows.view(b, h // window_size, w // (window_size * 2) , window_size, window_size * 2, -1)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = WindowAttention(
            dim,
            window_size=(self.window_size, self.window_size*2),
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            qk_scale=qk_scale,
            attn_drop=attn_drop,
    ...
    def forward(self, x, x_size, condition):
        ...
        if self.use_vit_condition:
            x = x.permute(0, 3, 1, 2)
            if self.window_condition:
                condition_wind = torch.stack(torch.meshgrid(torch.linspace(-1,1,12),torch.linspace(-1,1,24)))\
                    .type_as(x).unsqueeze(0).repeat(b, 1, h//self.window_size, w//(self.window_size*2))
                if self.shift_size > 0:
            ...
            x_windows_warped = x_windows_warped.view(-1, self.window_size * self.window_size*2, c)  # nw*b, window_size*window_size, c
        else:
