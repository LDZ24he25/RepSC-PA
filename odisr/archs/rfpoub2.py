...
class RepRFB(nn.Module):
    def __init__(self, feature_nums, act_type='lrelu', deploy=False):
        ...
        self.repblock2 = Rescpa(in_channels=feature_nums, out_channels=feature_nums, act_type=act_type, deploy=deploy)
...
class RepRFN(nn.Module):
    ...
    def forward(self, inputs, condition):
        ...
        inputs1 = flow_warp(inputs, flow, interp_mode='bilinear', padding_mode='border')
        outputs = self.reprfb1(inputs1)
        # outputs = flow_warp(outputs, flow, interp_mode='bilinear', padding_mode='border')
